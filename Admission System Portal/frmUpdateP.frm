VERSION 5.00
Object = "{67397AA1-7FB1-11D0-B148-00A0C922E820}#6.0#0"; "MSADODC.OCX"
Begin VB.Form frmUpdateP 
   Caption         =   "UPDATE PERSONAL DETAILS"
   ClientHeight    =   3030
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   4560
   LinkTopic       =   "Form1"
   ScaleHeight     =   10950
   ScaleWidth      =   20250
   StartUpPosition =   3  'Windows Default
   WindowState     =   2  'Maximized
   Begin VB.TextBox txtmothername 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "Mother_Name"
      DataSource      =   "Adodc1"
      Height          =   495
      Left            =   4200
      TabIndex        =   16
      Top             =   2640
      Width           =   2175
   End
   Begin VB.TextBox txtfathername 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "Father_Name"
      DataSource      =   "Adodc1"
      Height          =   495
      Left            =   4200
      TabIndex        =   15
      Top             =   1800
      Width           =   2175
   End
   Begin VB.TextBox txtlname 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "LName"
      DataSource      =   "Adodc1"
      Height          =   495
      Left            =   9960
      TabIndex        =   14
      Top             =   840
      Width           =   2175
   End
   Begin VB.TextBox txtmname 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "MName"
      DataSource      =   "Adodc1"
      Height          =   495
      Left            =   7080
      TabIndex        =   13
      Top             =   840
      Width           =   2175
   End
   Begin VB.TextBox txtfname 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "FName"
      DataSource      =   "Adodc1"
      Height          =   495
      Left            =   4200
      TabIndex        =   12
      Top             =   840
      Width           =   2175
   End
   Begin VB.TextBox txtsid 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "id"
      DataSource      =   "Adodc1"
      Enabled         =   0   'False
      Height          =   495
      Left            =   4200
      TabIndex        =   11
      Top             =   0
      Width           =   2175
   End
   Begin VB.OptionButton optopen 
      Appearance      =   0  'Flat
      Caption         =   "OPEN"
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   2400
      TabIndex        =   10
      Top             =   4320
      Value           =   -1  'True
      Width           =   2055
   End
   Begin VB.OptionButton optsc 
      Appearance      =   0  'Flat
      Caption         =   "SC/ST"
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   5280
      TabIndex        =   9
      Top             =   4320
      Width           =   2055
   End
   Begin VB.OptionButton optobc 
      Appearance      =   0  'Flat
      Caption         =   "OBC"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   8280
      TabIndex        =   8
      Top             =   4320
      Width           =   2055
   End
   Begin VB.OptionButton optph 
      Appearance      =   0  'Flat
      Caption         =   "PHYSICALLY HANDICAPPED"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   11520
      TabIndex        =   7
      Top             =   4320
      Width           =   3735
   End
   Begin VB.Frame frmPhone 
      Appearance      =   0  'Flat
      Caption         =   "PHONE NO."
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   2295
      Left            =   7320
      TabIndex        =   2
      Top             =   6360
      Width           =   4455
      Begin VB.TextBox txtmobile 
         DataField       =   "mobile_no"
         DataSource      =   "Adodc1"
         Height          =   495
         Left            =   1920
         TabIndex        =   4
         Top             =   480
         Width           =   2055
      End
      Begin VB.TextBox txtlandline 
         DataField       =   "landline_no"
         DataSource      =   "Adodc1"
         Height          =   495
         Left            =   1920
         TabIndex        =   3
         Top             =   1320
         Width           =   2055
      End
      Begin VB.Label Label18 
         Caption         =   "MOBILE"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   12
            Charset         =   0
            Weight          =   400
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         Height          =   375
         Left            =   240
         TabIndex        =   6
         Top             =   480
         Width           =   1455
      End
      Begin VB.Label Label19 
         Caption         =   "LANDLINE"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   12
            Charset         =   0
            Weight          =   400
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         Height          =   375
         Left            =   240
         TabIndex        =   5
         Top             =   1320
         Width           =   1455
      End
   End
   Begin VB.TextBox txtaddress1 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      DataField       =   "Address1"
      DataSource      =   "Adodc1"
      Height          =   975
      Left            =   3360
      MultiLine       =   -1  'True
      TabIndex        =   1
      Top             =   5160
      Width           =   8655
   End
   Begin VB.CommandButton cmddone 
      Caption         =   "NEXT"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   18
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   855
      Left            =   5520
      TabIndex        =   0
      Top             =   9120
      Width           =   2535
   End
   Begin MSAdodcLib.Adodc Adodc1 
      Height          =   375
      Left            =   13440
      Top             =   7200
      Visible         =   0   'False
      Width           =   1200
      _ExtentX        =   2117
      _ExtentY        =   661
      ConnectMode     =   0
      CursorLocation  =   3
      IsolationLevel  =   -1
      ConnectionTimeout=   15
      CommandTimeout  =   30
      CursorType      =   3
      LockType        =   3
      CommandType     =   2
      CursorOptions   =   0
      CacheSize       =   50
      MaxRecords      =   0
      BOFAction       =   0
      EOFAction       =   0
      ConnectStringType=   1
      Appearance      =   1
      BackColor       =   -2147483643
      ForeColor       =   -2147483640
      Orientation     =   0
      Enabled         =   -1
      Connect         =   "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=ARC1;Data Source=NAFISA-VAIO\SQLEXPRESS"
      OLEDBString     =   "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=ARC1;Data Source=NAFISA-VAIO\SQLEXPRESS"
      OLEDBFile       =   ""
      DataSourceName  =   ""
      OtherAttributes =   ""
      UserName        =   ""
      Password        =   ""
      RecordSource    =   "Student"
      Caption         =   "Adodc1"
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      _Version        =   393216
   End
   Begin VB.Label Label17 
      Appearance      =   0  'Flat
      Caption         =   "CATEGORY"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   255
      Left            =   120
      TabIndex        =   22
      Top             =   4440
      Width           =   1575
   End
   Begin VB.Label Label4 
      Appearance      =   0  'Flat
      Caption         =   "MOTHER NAME"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   495
      Left            =   0
      TabIndex        =   21
      Top             =   2640
      Width           =   2895
   End
   Begin VB.Label Label3 
      Appearance      =   0  'Flat
      Caption         =   "FATHER NAME"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   495
      Left            =   0
      TabIndex        =   20
      Top             =   1800
      Width           =   2295
   End
   Begin VB.Label Label2 
      Appearance      =   0  'Flat
      Caption         =   "NAME"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   0
      TabIndex        =   19
      Top             =   960
      Width           =   2055
   End
   Begin VB.Label Label20 
      Appearance      =   0  'Flat
      Caption         =   "ADDRESS"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   375
      Left            =   240
      TabIndex        =   18
      Top             =   5400
      Width           =   1935
   End
   Begin VB.Label Label1 
      Appearance      =   0  'Flat
      Caption         =   "ID"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   495
      Left            =   0
      TabIndex        =   17
      Top             =   120
      Width           =   1575
   End
End
Attribute VB_Name = "frmUpdateP"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private Sub cmdDone_Click()

 

 If optph.Value = True Then
   Adodc1.Recordset.Fields(8) = "Physically Handicapped"
   Else
   If optsc.Value = True Then
    Adodc1.Recordset.Fields(8) = "SC/ST"
    Else
     If optobc.Value = True Then
         Adodc1.Recordset.Fields(8) = "OBC"
         Else
           Adodc1.Recordset.Fields(8) = "Open"
      End If
    End If
End If

 Adodc1.Recordset.Update
 Adodc1.Refresh
 MDIfrmstud.Show
 Me.Hide
 
 
End Sub

Private Sub Form_Load()
Adodc1.Refresh
Adodc1.Recordset.MoveFirst
 Do Until Adodc1.Recordset.EOF
  If Adodc1.Recordset.Fields(0) = j Then
   Exit Do
  End If
  Adodc1.Recordset.MoveNext
  Loop
If Adodc1.Recordset.Fields(8) = "Physically Handicapped" Then
 optph.Value = True
Else
 If Adodc1.Recordset.Fields(8) = "OBC" Then
  optobc.Value = True
 Else
  If Adodc1.Recordset.Fields(8) = "SC/ST" Then
   optsc.Value = True
   Else
   optopen.Value = True
  End If
   End If
   End If
  
 

End Sub
